...
A<-ggplot(data=dataa,aes(x=ABETA,y=PTAU))+ 
  ...
  geom_point(aes(shape=class),size=1) + xlim(75,310)+ylim(0,150)+theme_bw()+
...
Ad<-rbind(SMOTE(x,floor((nrow(MCI)-nrow(AD))/2),5),x)
batt(x,Ad)
dataa<-data.frame(Ad,class='AD')
...
Ad<-rbind(SMOTE(x,(nrow(MCI)-nrow(AD)),5),x)
batt(x,Ad)
dataa<-data.frame(Ad,class='AD')
class<-c(rep("AD",nrow(dataa)))
...
D<-ggplot(data=dataa,aes(x=ABETA,y=PTAU))+ 
  ...
  scale_alpha_continuous(limits=c(0,0.2),breaks=seq(0,0.2,by=0.05))+
  scale_color_manual(values=c("AD"="gray"),guide=FALSE)+
  geom_point(aes(shape=class),size=2) + xlim(75,310)+ylim(0,150)+theme_bw()+
...
cairo_ps(width=13,height = 3,file='plot6.eps')
