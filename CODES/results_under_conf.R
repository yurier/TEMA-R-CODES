#library(data.table)
#
#lista_best_under<-as.data.frame(NULL)
#for (i in 2:9){
#  lista_best_under<-rbind(answer_under[[2]][answer_under[[2]][,4]==i,][1,],lista_best_under)
#}

#lista_best_under<-lista_best_under[order(lista_best_under[,6]),]
  s<-8
  nameee<-rownames(as.data.frame(answer_under[[1]][lista_best_under[,6][s]]))
  ...
  PLASMA<-subset(PLASMA, select=c("DX",nameee))
  ...
  i<-1
    for (j in c(1:CV)){
      ...
      ans1 <- knn(train,test,cltra,lista_best_under[s,7])
      modtest<-ans1
  ...
  matrix_8_under<-matrix_aux
  #lista_best_under[s,7]<-which.max(test_mean)
  #lista_best_under[s,8]<-test_mean[which.max(test_mean)]
  #lista_best_under[s,9]<-test_sd[which.max(test_mean)]