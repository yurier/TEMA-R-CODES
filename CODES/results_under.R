...
for (i in 2:7){
  lista_best_under<-rbind(table_it_under_nondef[table_it_under_nondef[,4]==i,][1,],lista_best_under)
}

lista_best_under<-lista_best_under[order(lista_best_under[,6]),]

for (s in 1:nrow(lista_best_under)){
  ...
  PLASMA1<-PLASMA1[,c(combn(1:9,lista_best_under[s,4])[,lista_best_under[s,5]])]
  ...
  lista_best_under[s,8]<-which.max(test_mean)
  lista_best_under[s,9]<-test_mean[which.max(test_mean)]
  lista_best_under[s,10]<-test_sd[which.max(test_mean)]
  print(lista_best_under)
}