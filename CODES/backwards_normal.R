backward_normal <- function(data,bignun,neigh){
  ...
  vec<-c(test[,2]) #inicio com todas as variaveis
  ...
  while(crit==1){
    ...
    if(tab[which.max(tab$max_hit),2]>=bst)
    {
      ...
      table_it<-NULL
      bst<-tab[which.max(tab$max_hit),2]
      vec<-vec[vec!=test[tab[which.max(tab[,2]),3],2]] #nova lista de variaveis 
  ...
  return(list(vec,tab,proc.time() - ptm,bst,tab2))
